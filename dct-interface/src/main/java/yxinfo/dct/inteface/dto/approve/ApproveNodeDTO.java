...
public class ApproveNodeDTO extends BaseDTO {
    ...
    private static final long serialVersionUID = 3540031739317595887L;

    private Integer id;

    // 审核人id
    @NotNull( groups = Add.class, message = ErrorCode.APPROVE_NODE_MEMBERID_NOT_EMPTY )
    private Integer memberId;

    // 状态（s审核通过, f审核不通过）
    @Length( groups = Add.class, max = 1, message = ErrorCode.APPROVE_NODE_STATUS_LENGTH )
    @NotNull( groups = Add.class, message = ErrorCode.APPROVE_NODE_STATUS_NOT_EMPTY )
    ...
    // 审核意见
    @Length( groups = Add.class, max = 255, message = ErrorCode.APPROVE_NODE_OPINION_LENGTH )
    private String opinion;

    // 审核流程记录id
    @NotNull( groups = Add.class, message = ErrorCode.APPROVE_NODE_ARID_NOT_EMPTY )
    ...
    // 是否结束（自由模式专用, 1时结束流程）
    @Length( groups = Add.class, max = 1, message = ErrorCode.APPROVE_NODE_ISEND_LENGTH )
